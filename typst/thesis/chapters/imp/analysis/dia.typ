...
chose to model.==== Variables and indicator functions<subsubsection-imp-dia-vars>
...
Important properties of ```hs IndicatorFunction```s (and thus of ```hs
VarsSet```s) follows.
#theorem(
  name: "Equivalence of indicator functions",
    ...
    ```hs if-ext : ∀ {s₁ s₂ : IndicatorFunction} -> (a-ex : ∀ x -> s₁ x ≡ s₂ x) -> s₁ ≡ s₂```
    <thm-if-equiv>]
#theorem(name: "Neutral element of union")[

  ```hs ∪-∅ : ∀ {s : IndicatorFunction} -> (s ∪ ∅) ≡ s```
  <thm-if-neutral-union>]

#theorem(name: "Update inclusion")[

  ```hs ↦=>⊆ : ∀ {id} {s : IndicatorFunction} -> s ⊆ (id ↦ s)```
]
...
... | just _ = true
... | nothing = false
```, caption: [Code to compute the domain of a ```hs Store``` in Agda])<code-store-domain>
...
#figure(
  ...
  data Dia : VarsSet -> Command -> VarsSet -> Set where
   ...
   while : ∀ b v v₁ c -> (b⊆s : (bvars b) ⊆ v) -> 
      ...
      ```,
  caption: "Dia relation in Agda",
)<code-dia>

What we want to show now is that if ```hs Dia``` holds, then the evaluation of
a command $c$ does not result in an error: while #thmref(<thm-adia-sound>)[Theorem] and 
#thmref(<thm-bdia-sound>)[Theorem] show that if the variables in an arithmetic expression or 
a boolean expression are contained in a store the result of their evaluation can't be a failure
(i.e. they result in "just" something), #thmref(<thm-cdia-sound>)[Theorem] shows that 
if ```hs Dia``` holds, then the evaluation of a program failing is absurd.
...
#theorem(name: "Soundness of definite initialization for commands")[
```hs
